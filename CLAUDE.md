...
**After coding — mandatory before any PR:** run `npm test`, then `/simplify` (reuse/simplification/efficiency pass on the changed code), then `/verify` (exercise the change end-to-end). For larger changes, also run `/code-review`. Do not open the PR until all pass clean.

## Writing & changing code

- **Code quality principles (above) apply to all of the following** — the mechanics here never excuse skipping the design/simplicity bar.
