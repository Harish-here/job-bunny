...
- **`profiles/harish/` carries inert legacy v0 config files** (`filter_config.json`, board `.md`s, `resume_meta.json`) alongside the live v2 set — migration confirmed complete 2026-07-26 (`scripts-v2-migrate/migrate.ts --profile harish` dry-run: no missing v2 keys; only remnant is avoid.md's 2-entry alias map, which has no v2 home). Never read or run the pipeline against it — real user data.
