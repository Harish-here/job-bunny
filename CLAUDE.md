...
- **`profiles/harish/` still carries legacy v0 config files** alongside `filter.json` — run `scripts-v2-migrate/migrate.ts --profile harish` (dry-run by default) to confirm/complete migration. Never read or run the pipeline against it — real user data.
