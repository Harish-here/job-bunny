...
- `builders.ts` — live connector/notifier/routine/lane construction (`buildConnector`, `buildNotifier`, `buildRoutine`, `buildLanes`, `buildLinkedInLane`), plus (2026-08-02) `missingTokenNotionClient` (moved here from `compose.ts`) and the `wireMigrate`/`MigrateWire` composition seam for `jobbunny migrate` (a narrow Notion-read + lazily-opened sqlite-import handle, no full `wire()`), split out of `compose.ts` purely to keep it under the file-size cap.
...
| `core/tracking` | Single authority for the tracking-status vocabulary: `STATUS_OPTIONS`, `PASSED_STATUS`, `TrackingStatus`. Byte-exact relocation target — still pinned against the live Notion DB's select options (CLAUDE.md hard rule); `adapters/db/notion/schema.ts` re-sources `STATUS_OPTIONS` from here instead of defining it, and `SqliteConnector`'s archive policy keys staleness on `PASSED_STATUS` from here. 2026-08-02: `fields.ts` adds `TrackingFieldsSchema`/`TrackingFields` (the 7 manual Notion columns' local home, all-optional, owned by the future app server) and `MigratedRecord` (`{jd, tracking?}`, the `jobbunny migrate` hand-off shape) | `status.ts`, `fields.ts` |
...
### `src/cli/`
`main.ts` (bin entry; `import 'dotenv/config'` **first and only here** — a daemon-spawned scheduled run hands a minimal env), `wire/` (§2.3), `commands/`: run, doctor, reconcile, stage, routine, serve, autostart, lane, profile, setup, release, migrate (2026-08-02). (`schedule` was deleted 2026-07-27 — see §2.1/§6.) Commands **return** exit codes; only the bin guard touches `process.exitCode`.
...
**CLI surface** (`src/cli/main.ts` USAGE): run / doctor / reconcile / stage / routine / serve start|stop|status (cross-profile) / autostart enable|disable (cross-profile, darwin only) / lane add-url / profile build|remove / setup / release / migrate [--apply] (2026-08-02: one-shot Notion → local sqlite import, dry-run by default; `--apply` imports insert-only then flips `profile.json`'s `connector` to `sqlite`). `--profile` required except `serve`, `autostart`, and `release`.
