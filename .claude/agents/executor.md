...
- **Pure domain logic, zero I/O** (a schema, a scorer, a pure transition function) → `src/core/<module>/`. Existing modules for reference: `core/jd` (the universal JD schema), `core/config`, `core/filter`, `core/dedup`, `core/rank`, `core/company`, `core/errors`, `core/async`, `core/personas` (`core/profile` was deleted — its `ResumeSchema` had zero importers). No `Date.now()`/`Math.random()` inside scorers; pass `now` in as a parameter.
- **A new capability interface** (something pipeline code needs to call but shouldn't know the concrete implementation of) → `src/ports/`. Existing ports: `board` (`BoardStore`/`BoardSource`), `board_daemon`, `board_preview`, `browser`, `checkpoint_store`, `config_store`, `connector`, `context`, `doctor`, `lane` (`FarmingLane`/`ApiLane`), `llm`, `notifier`, `run_intents`, `run_store`, `state_store`, `storage`. Interface only, no implementation, and it may import nothing but `core`.
...
- **Frontend/UI code** (React components, the browser-side API client) → `ui/<...>` — a separate npm workspace outside `src/`, with its own gate (`ui:check` = `tsc --noEmit && tsc -p tsconfig.e2e.json --noEmit && biome check src e2e playwright.config.ts && vitest run`, `ui:build` = vite build) run by CI's separate `ui` job, not `npm run check`; the boundary rules below apply to `src/**` only, never to `ui/`; the file-size caps (400 impl / 800 test) cover `src/`, `test/`, `ui/src/`, and `ui/e2e/` — a `.tsx` component is capped exactly like a `.ts` module. It reaches backend contract types solely via a type-only import of `src/app/features/*/index.ts` (`ui/src/lib/api/types.ts`); it may additionally import specific dependency-free `src/core/**` modules at runtime (currently `core/datetime`, `core/normalize_token`, `core/tracking/vocab.ts` — see CLAUDE.md's ui→core seam paragraph), never any other `src/` code. Design tokens/colours/type ramp/voice rules are authored in `docs/product/ui-design-system/reference.md`, machine-checked against `ui/src/index.css` and reserved-word usage by `ui/src/lib/tokens.test.ts` — treat it as the design-system source of truth for UI work, not a convention to reinvent per-component.
