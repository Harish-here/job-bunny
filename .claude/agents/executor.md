...
- **Frontend/UI code** (React components, the browser-side API client) → `ui/<...>` — a separate npm workspace outside `src/`, with its own gate (`ui:check` = `tsc --noEmit && tsc -p tsconfig.e2e.json --noEmit && biome check src e2e playwright.config.ts && vitest run`, `ui:build` = vite build) run by CI's separate `ui` job, not `npm run check`; the file-size caps and boundary rules below apply to `src/**` only, never to `ui/`. It reaches backend contract types solely via a type-only import of `src/app/features/*/index.ts` (`ui/src/lib/api/types.ts`) — never a runtime import of anything under `src/`.
