...
**Revision log**: second review round (this pass) closed six blocker/should-fix findings against the live code: the daemon pidfile's staleness rule wrongly reused `run_lock.ts`'s 4-hour age check (D22 replaces it with a heartbeat); `serve stop` killed the in-flight child before the daemon, letting the daemon spawn the next owed run mid-stop and orphan it (D10 inverted to daemon-first); a later batch entry's grace window could silently expire behind a long-running predecessor with no revalidation (§4.3/§6.4); in-place pidfile updates used a non-atomic `writeFileSync` that concurrent readers could observe mid-write (§6.2); spawned-child `error` events were unhandled and would kill the daemon (§4.3/§6.1/§9.1); and the run-cap figure in §6.5 was hardcoded and arithmetically wrong (now derived live via `computeRunCapMs`).

**Revision log (third pass)**: this pass corrects a sequencing defect in D22's heartbeat mechanism, found during implementation review, that would have silently defeated the second pass's own fix. The heartbeat write was specified inside the reentrancy-guarded tick body; because the guard short-circuits every tick fired while a child run is in flight, `lastTickAt` would have frozen for the entire duration of any multi-hour run and made the 5-minute staleness threshold unsafe. Closes four related findings: the heartbeat write now precedes the reentrancy guard and runs on every timer firing, guarded-body-or-not (§6.6, §6.2, D22, A15.1); pidfile updates are specified as synchronous `writeFileSync`+`renameSync`, not the async `run_folder.ts` pattern A4 originally pointed at, because the non-interleaving guarantee the reordering depends on does not hold for async writes (§6.2, correcting A4, A15.2); a heartbeat write failure is caught, logged, and swallowed rather than left to crash the daemon (§6.6, §9.1, A15.3); and the steal path at `serve start` waits one tick interval plus margin and re-checks before stealing a pidfile whose heartbeat looks stale, so a machine waking from sleep is not mistaken for a dead daemon (§6.2, §6.8, D22, A15.4/A15.5).

**Revision log (consolidated review round)**: this pass closes a cross-plan review against the foundation plan (Tasks 3, 4, 6). It corrects §7.2's resolution-order paragraph, which had wrongly attributed the three-tier Chrome-candidate resolution to `wire.ts`; the resolution runs inside `launchChrome` itself (via the injectable `LauncherDeps.env`), a deliberate placement so the env override applies on every launch path and the whole chain stays hermetically testable. No decision registers or tier ordering changed.
...
| D10 | `serve stop` semantics | Kills the **daemon first**: `SIGTERM` → poll (not a fixed sleep) until dead or `SIGKILL_GRACE_SECONDS` (20s) elapses → `SIGKILL` → poll again. Only then RE-READS the pidfile for `inFlight` and applies the identical escalation to it if a pid is recorded and alive. Only once both are confirmed dead does it remove the pidfile and exit 0; if either survives the `SIGKILL` step, `serve stop` exits nonzero, leaves the pidfile in place, and prints the surviving pid(s). The in-flight child pid is recorded in the pidfile so it stays killable even if the daemon process itself has died. | Killing the child first (the original order) lets the daemon's own `await` on that child resolve and its tick/batch loop spawn the NEXT owed run before `serve stop`'s `SIGTERM` reaches the daemon — orphaning a non-detached child, since the signal only ever targets the daemon's pid, not a process group. Daemon-first prevents this by construction: once the daemon is dead it cannot spawn or write to the pidfile again, so the pidfile's last write is authoritative when re-read for `inFlight`. Windows caveat: Node emulates `SIGTERM` on Windows as an unconditional terminate, so the daemon cannot let the child clean up gracefully there. Accepted — the daemon holds no unflushed state (the run-folder ledger lives on disk) and `--resume` already handles an incomplete checkpoint. |
...
| D21 | Daemon and run-child logging | `~/.jobbunny/logs/daemon.log` (the detached daemon's own stdout/stderr) and `~/.jobbunny/logs/runs.log` (every spawned run child's stdout/stderr) — both opened with `fs.openSync(path, 'a')` and passed as `stdio: ['ignore', fd, fd]` in place of D5's original `stdio: 'ignore'` for those two spawn calls. One shared `runs.log` is sufficient because D6 mandates sequential execution — only one child ever writes at a time. Rotation is asymmetric (A6, §6.9): `runs.log` (>10MB → renamed to `<name>.1`, replacing any existing `.1`, fresh file reopened) is checked and rotated BEFORE EACH child spawn, not only at daemon start, because a healthy daemon now runs for months (D20 autostart) and start-only rotation would let it grow unboundedly; `daemon.log` keeps start-only rotation, because its fd is fixed at the detached spawn and can't be re-handed to a running process, renaming an open-handle file fails on Windows, and it stays acceptable since the daemon writes to `daemon.log` on events only (spawn, child exit, skipped expired slot, spawn error, start, stop), never once per tick. | D16 named the `~/.jobbunny/logs/` destination but neither §6.1/§6.3's `stdio: 'ignore'` spawns nor §4.3 step 5c's unstated child stdio gave it a mechanism — the daemon and every run it spawns would otherwise produce zero diagnostics. This is also what makes B1's three pre-checkpoint abort paths (which only `console.error`, with no run folder) diagnosable at all. |
| D22 | Daemon pidfile staleness (heartbeat, not age) | Daemon pidfile gains `lastTickAt: string` (ISO 8601), written as the FIRST statement of the tick callback — BEFORE the reentrancy guard (§6.6, A15.1) — so it is rewritten on EVERY 30s timer firing, including firings the guard short-circuits while a child run is in flight; a write failure is caught, logged to `daemon.log`, and swallowed rather than left to crash the daemon (A15.3). All pidfile updates — the heartbeat, `inFlight` set/clear, and the `attempts` ledger append — use the SYNCHRONOUS `writeFileSync`-then-`renameSync` atomic form, not the async temp+rename pattern originally specified (A15.2, correcting A4) — required so the heartbeat write outside the guard can never interleave with the guarded body's own pidfile updates. Staleness (safe to steal at `serve start`) is: recorded `pid` not alive, OR `lastTickAt` older than 5 minutes (10 missed ticks) — this REPLACES reuse of `run_lock.ts`'s 4-hour `DEFAULT_MAX_AGE_MS` for the daemon pidfile specifically. When the pid IS alive but the heartbeat looks stale, `serve start` does not steal immediately: it waits one tick interval plus margin (35 seconds), re-reads the pidfile, and steals only if `lastTickAt` still has not advanced (A15.4); a pid that is NOT alive still steals immediately, no wait. `run_lock.ts`'s own 4-hour rule remains correct for `run_lock` (a single bounded run) and is simply the wrong rule for a long-lived daemon. | §6.2 previously reused `run_lock.ts`'s `isStale` rule verbatim (pid dead OR `startedAt` older than `DEFAULT_MAX_AGE_MS` = 4h). The daemon writes `startedAt` once and runs for days, so from hour 4 onward every `serve start` would judge a healthy daemon stale and start a second one — which §9.1's "no second daemon is ever allowed" row forbids. A heartbeat ties staleness to liveness (is it still ticking?) rather than to uptime (how old is it?), which is the property that actually matters for a long-lived process. A heartbeat written inside the guard instead of before it would freeze for the duration of every child run and defeat this rule entirely (A15.1) — the exact failure this decision exists to prevent, just moved one layer down; synchronous updates (A15.2) are what make that reordering race-free in practice, not just on paper. The 35-second re-check (A15.4) exists because `setInterval` does not fire during system suspend (D4), so a healthy daemon can legitimately show a stale heartbeat for up to one tick interval immediately after wake; re-checking once distinguishes that from a genuinely dead or wedged daemon. |
| D23 | Same-change documentation obligation | This port updates, in the same change that ships it: `CLAUDE.md` (the "What this is" platform line, the Commands section, the Profiles section's `schedule install` cross-profile exception, the Pipeline architecture section wherever it names the scheduler adapter, and the Known limitations section if affected); `.claude/agents/explainer.md` (the scheduling, CLI-surface, and adapter-inventory portions of the baked-in KB); `.claude/agents/executor.md` (checked, and updated if it references the scheduler adapter or the `schedule` subcommands); and `README.md` (the Windows/Linux manual autostart alternative D20 promises, plus the `claude` CLI prerequisite from D13). See §12. | CLAUDE.md states "macOS only (launchd scheduling, hardcoded Chrome path)" and "`--profile <name>` is required on every command except `schedule install` (cross-profile by design)" — both become false once this ships. CLAUDE.md's own hard rule is that architecture docs are code, updated in the same change that alters behavior, not as a follow-up — and this port deletes an entire adapter family and changes documented platform scope, so the obligation is stated explicitly rather than assumed. |
...
5. For each `OwedRun` returned, in ascending `(slot, profileName)` order (A13 — the daemon sorts this explicitly; it no longer inherits ordering from `collectScheduledJobs`'s sorted-profile-name convention (`cli/commands/schedule.ts:99`) by construction, since D14 deletes that file), the daemon:
   a. **Revalidates** the slot's grace window (A3): re-checks `now <= slot + graceMinutes` (§5.1 rule 5) immediately before acting on this entry. If expired, skips it — logs the skip, does NOT append it to the attempts ledger (it was never attempted), and proceeds to the next `OwedRun`. This guards against a slot's grace window expiring behind an earlier, still-running sequential child in the same batch (a single child can run as long as the backstop allows, §6.5) — see §6.4 and §9.1 for why revalidation must happen per entry, not once per tick. Order for this entry from here is **revalidate → ledger append → spawn**.
   b. Appends `{ profile, date, slot }` to the pidfile's `attempts` ledger (D19) and persists the pidfile — BEFORE spawning, so a crash between this append and the spawn still counts the slot as attempted (§5, §6.2).
   c. Spawns `node src/cli/main.ts run --profile <name> --headless` via `node:child_process.spawn` (not detached — the daemon supervises and awaits this child; only Chrome itself, launched inside that child's own `wire()` call, is `detached`+`unref`ed, unchanged from today), with `stdio: ['ignore', runsLogFd, runsLogFd]` (D21, §6.9) — after checking `runs.log`'s size and rotating it if needed, the safe quiet point for that check (A6, §6.9) — and with an `error` event handler attached to the returned `ChildProcess` (A7): a spawn `error` (e.g. `ENOENT`, `EMFILE`) is treated exactly as a nonzero child exit; 5b's ledger append already prevents a retry storm here (D19).
   d. Once `spawn()` returns a pid, writes it into the daemon pidfile's `inFlight` field (`src/ops/daemon/pidfile.ts`) — the pid does not exist until `spawn()` returns, so this is necessarily after, not before, the spawn call (S1). The resulting gap — a daemon crash between spawn and this write orphans an unrecorded child — is accepted; §9's existing `run_lock.ts` tolerance already covers an untracked orphan. This is distinct from 5b's attempts-ledger append, which is deliberately before the spawn and records the *slot*, not the pid.
   e. Awaits the child's exit (any exit code — D7: failure still counts the slot as served) or the spawn `error` handler from 5c firing instead.
   f. Clears `inFlight` from the pidfile.
   g. Sequentially proceeds to the next `OwedRun` in the batch — never spawns two children concurrently (D6).
...
| Command | Behavior |
|---|---|
| `jobbunny serve start` | Split into a **parent** and a **child** process (S3), so the child never fights the parent for the pidfile. **Parent** (no flag): on darwin, first checks for stale `launchd` plists (D15/§8, narrowed pattern per D20) — refuses if any are found. Otherwise acquires the daemon pidfile (exclusive `wx` create, same mechanism as `ops/scheduling/run_lock.ts`'s `acquireRunLock`) — refuses (prints who holds it) if another daemon is already running (a live pid in the pidfile). The `wx`-create writes the PARENT's own pid as a placeholder (A10, §6.1) — a concurrent `serve start` reading the pidfile in the brief window before the child is recorded sees a live pid and correctly refuses; if the parent crashes before spawning, the placeholder is a dead pid, which D22's heartbeat-based staleness rule treats as stale, so a later `serve start` self-heals with no manual cleanup. Spawns the detached child (`spawn(process.execPath, [mainPath, 'serve', 'start', '--daemon-child'], { detached: true, stdio: ['ignore', daemonLogFd, daemonLogFd] })` + `unref()` — §6.9) with an `error` event handler attached (A7): a spawn `error` (e.g. `ENOENT`, `EMFILE`) follows the identical already-dead path below — the parent REMOVES the pidfile it created, prints the tail of `daemon.log`, and exits 1. On a successful `spawn()`, overwrites the pidfile's placeholder pid with the child's pid via the atomic in-place update path (A4, §6.2) it already holds — necessarily after `spawn()` returns, since the pid does not exist before that. Waits up to 2 seconds, confirming the child is alive via `process.kill(childPid, 0)`. On success, exits 0. If the child is already dead, the parent REMOVES the pidfile it created, prints the tail of `daemon.log`, and exits 1. **Child** (`serve start --daemon-child`): does NOT acquire the pidfile — the parent already owns it and already recorded this process's pid into it; without this split the child would hit `EEXIST` on the pidfile's `wx` create and refuse to start. The child's only job is to run the tick loop. |
| `jobbunny serve stop` | **Daemon-first order (D10, A2)**: killing the child before the daemon would let the daemon's own `await` resolve and spawn the NEXT owed run before the `SIGTERM` reaches it, orphaning that non-detached child (the signal targets only the daemon's pid, not a process group). Sequence: (1) Read the pidfile for the daemon pid. Send `SIGTERM`. POLL (not a fixed sleep) until the daemon is dead or `SIGKILL_GRACE_SECONDS` (20 seconds — the same constant the retired plist watchdog used, F1/§6.5) elapses; then `SIGKILL`, and poll again. If still alive after that: exit nonzero, leave the pidfile in place, print the pid. (2) RE-READ the pidfile for `inFlight` — safe because a dead daemon cannot spawn or write to the pidfile again, so the pidfile's last write is authoritative. If an `inFlight` pid is recorded and alive, apply the identical `SIGTERM` → poll up to 20s → `SIGKILL` escalation to it. (3) Only once both are confirmed dead: remove the pidfile, exit 0. `ESRCH` at any step is treated as already-dead, not an error. |
...
interface DaemonPidfile {
  ...
  lastTickAt: string;    // ISO 8601 — rewritten by the daemon on EVERY 30s tick (D22),
                          // as the FIRST statement of the tick callback, BEFORE the
                          // reentrancy guard (§6.6, A15.1), through the synchronous
                          // atomic in-place update path below (A15.2)
  inFlight?: {
  ...
  attempts: Array<{ profile: string; date: string; slot: string }>; // D19 — appended
    // to BEFORE each spawn for an owed slot (§4.3 step 5b); cleared on local-date rollover
}
```

**Mechanism**: identical shape to `run_lock.ts`'s `RunLockDeps`/`acquireRunLock`/`releaseRunLock` — an injectable `PidfileDeps` (`{ path, pid, now, pidIsAlive, readFile, writeFileExclusive, updateFile, unlink }`) with a real default (`defaultPidfileDeps`) built from `node:fs`'s `readFileSync`/`writeFileSync({flag:'wx'})`/`unlinkSync` and `process.kill(pid,0)`. The INITIAL acquisition stays `wx`-exclusive create, unchanged — that is the mutual-exclusion guarantee, exactly mirroring `run_lock.ts`'s own `tryCreate`. Unlike `run_lock.ts` (whose file is create-once, delete-on-release, no in-place update), the daemon pidfile is **updated in place** repeatedly over its lifetime — `inFlight` set before each spawn and cleared after each exit, `attempts` appended to before each spawn (D19), and `lastTickAt` rewritten every 30s tick (D22).

**Atomic in-place updates (A4, corrected by A15.2)**: every one of those in-place updates — `inFlight`, `attempts`, `lastTickAt` — goes through `updateFile`'s temp-file-plus-rename implementation, SYNCHRONOUSLY: `writeFileSync('<path>.tmp', ...)`, then `renameSync('<path>.tmp', path)` over the target. This is a deliberate divergence from `ops/observability/run_folder.ts:155-160`'s `writeAtomic` — same temp-file-plus-rename shape, but the synchronous variant, not a verbatim copy of it, and the divergence is required, not incidental. A4 originally pointed at `run_folder.ts:155-160` as "the exact pattern" to follow; that function is `async` (`await writeFile` → `await rename`), which is the wrong form for the pidfile specifically: two in-flight async writes to the same file — for instance the heartbeat write A15.1 places outside the reentrancy guard, racing against the guarded body's own `inFlight`/`attempts` update — can both be mid-flight at once and collide on the same `<path>.tmp` path, and A15.1's non-interleaving guarantee does not hold for async writes. On Node's single-threaded event loop, a synchronous write-then-rename runs to completion without yielding control back to the event loop, so the heartbeat write can never interleave with the guarded body's own pidfile update, and no two writers can ever collide on the temp path — this is what makes A15.1's ordering safe in practice, not merely on paper. This REPLACES the plain `writeFileSync` (no `wx`, no rename) the pidfile's in-place updates originally used: that form truncates the file before writing, and §6.8 makes EVERY `jobbunny` command — including the daemon's own run children — a concurrent reader of this file, so a truncate-then-write left a window where a reader could observe empty or partial JSON. `run_lock.ts` never had this problem because its file is create-once and never updated in place, so "same mechanism as `run_lock.ts`" never covered updates — only the initial `wx` create, which is unaffected by this correction (`writeFileSync({flag:'wx'})` was already synchronous). This is mandatory rather than nice-to-have specifically because D22's heartbeat writes at least once every 30 seconds for the daemon's entire lifetime, so an in-place update is now a near-constant occurrence, not an occasional one. `attempts` starts as `[]` when the pidfile is first created and is reset to `[]` in place whenever the daemon detects local-date rollover, using the same synchronous atomic update path — see §5's "Attempts ledger" note (A9) for why this needs no new field.

**Reader rule, stated once (A4)**: a reader that fails to parse the pidfile retries once after a short delay, then treats it as corrupt, taking the same "unreadable ⇒ stale ⇒ safe to steal" posture `run_lock.ts:105` already documents for its own lock file. §6.8 suppresses its liveness warning on a parse failure, rather than misreporting the daemon as either running or down.

**Staleness (D22)**: a daemon pidfile is judged stale (safe for a new `serve start` to steal) when the recorded `pid` is not alive, OR `lastTickAt` is older than 5 minutes (10 missed ticks at the 30s cadence). This REPLACES reuse of `run_lock.ts`'s two-part `isStale` rule (pid dead OR the file older than the 4-hour `DEFAULT_MAX_AGE_MS`): the daemon writes `startedAt` once and then runs for days, so an age check against `startedAt` alone would judge every healthy daemon stale from hour 4 onward and let a new `serve start` steal a live daemon's pidfile — starting a second one, which §9.1's "no second daemon is ever allowed" row forbids. `run_lock.ts`'s own 4-hour `DEFAULT_MAX_AGE_MS` rule remains correct for `run_lock` itself (a single bounded run, not a long-lived process) — it is simply the wrong rule for the daemon pidfile, which is why the two pidfiles now use different staleness rules despite sharing an on-disk convention.

**Steal re-check (A15.4)**: the staleness rule above governs whether a pidfile is a CANDIDATE to steal; it does not mean `serve start` steals immediately on that first observation. When the recorded `pid` is ALIVE but `lastTickAt` is older than 5 minutes, `serve start` does NOT steal yet — it waits one tick interval plus margin (35 seconds — the 30s tick period plus a 5-second margin), re-reads the pidfile, and steals only if `lastTickAt` still has not advanced past the value it observed the first time. If the recorded `pid` is NOT alive, this re-check is skipped entirely and `serve start` steals immediately, with no wait. Rationale: `setInterval` timers do not fire during system suspend — the same monotonic-clock fact D4 cites for why the daemon avoids timer-to-next-fire scheduling — so immediately after a machine wakes, a healthy, ticking daemon can legitimately show a heartbeat that is hours stale, for up to one tick interval, purely because it has not yet had a chance to tick. The 35-second re-check is what distinguishes "dead or wedged" (heartbeat still frozen 35 seconds later) from "just woke" (heartbeat has advanced), without making every `serve start` wait the full 5 minutes on every wake.

### 6.3 Detach mechanism

Per D5: `spawn(command, args, { detached: true })` then `.unref()` — the same detach primitives as `launcher.ts`'s `launchChrome` (`spawn(chromePath, argv, { detached: true, stdio: 'ignore' }); child.unref();`); the daemon detaches itself from its own launching `serve start` invocation the same way Chrome is detached from the pipeline run that launches it. Chrome's own spawn keeps plain `stdio: 'ignore'`, unchanged; the daemon's own two spawn calls (this one, and the run-child spawn in §4.3 step 5c) use an append-fd `stdio` instead (D21, §6.9), so their diagnostics land in `~/.jobbunny/logs/` instead of being discarded.
...
When `isRunOwed` returns multiple `OwedRun`s in one tick — because multiple profiles each have an owed slot, because a single profile has multiple slots owed simultaneously (D8: possible when that profile's `graceMinutes` exceeds its own smallest inter-slot gap), because a catch-up tick finds several distinct owed slots at once after downtime, or any combination of the above — the daemon spawns and awaits each child **one at a time**, in ascending `(slot, profileName)` order (A13 — the identical phrase used in §4.3 step 5) — never concurrently. This ordering is now the daemon's own explicit sort, not an inherited property: today `(slot, profileName)` order coincides with `collectScheduledJobs`'s sorted-profile-name, file-order convention (`cli/commands/schedule.ts:99`) purely by coincidence, and D14 deletes that file entirely, so nothing else would supply this ordering once it's gone. This directly ports the reasoning behind today's `;`-joined single-`launchd`-job-per-slot plist command (`adapters/scheduler/launchd/plist.ts`'s `buildCommand`) and behind `ops/scheduling/run_lock.ts`'s existence: exactly one Chrome/CDP session exists (port 9222, one `.chrome-debug/` user-data-dir) — two concurrent `jobbunny run` invocations would fight over it and corrupt each other's harvest.
...
**Per-slot revalidation (A3)**: sequential execution means a single child can run for as long as the backstop allows it (~3h37m under current defaults, §6.5), so a later batch entry's own 90-minute grace window can already have expired by the time its turn arrives — without a check, the daemon would spawn it anyway, violating D8's "a 09:00 slot can never fire in the evening". §4.3 step 5's per-entry order makes this explicit: **revalidate → ledger append → spawn**. Immediately before acting on each `OwedRun`, the daemon re-checks `now <= slot + graceMinutes` (§5.1 rule 5); if it has expired, the entry is skipped and logged, and — critically — is NOT appended to the attempts ledger, since it was never attempted and `isRunOwed` will not return it again once its own grace window has lapsed (§5.1 rule 5 again, evaluated independently on the next tick). See §9.1's grace-expiry row for the resulting second cause of silent expiry this introduces, and §11 for the accepted trade-off.
...
The daemon ADDITIONALLY arms a `setTimeout` backstop when it spawns a child, set to that child's run cap plus a 5-minute margin. This is a faithful **port**, not a new policy, of the embedded BASH WATCHDOG the retired plist carries inside `buildCommand` (`adapters/scheduler/launchd/plist.ts:145-175`, deleted per D14) — the plist emits no timeout-related `launchd` key at all; the backstop was always this shell script, never a declarative plist property. The real mechanism: the plist's `buildCommand` runs the chained profile commands as a background job group, then `sleep ${backstopSeconds}` → `kill -0 "$PGID"` → `kill -SIGTERM -- "-$PGID"` → `sleep 20` → `kill -SIGKILL -- "-$PGID"`, where `backstopSeconds = Math.ceil(runCapMs / 1000) + 300` (`plist.ts:189`) and the grace is `SIGKILL_GRACE_SECONDS = 20` (`plist.ts:51`).

The daemon's backstop uses the SAME `+300s` margin as the retired watchdog, so this is a like-for-like port, not a new number. Concretely, under the default config, `computeRunCapMs` (`cli/commands/run.ts:71-79`, `RUN_CAP_MARGIN = 1.25`, sum of `stage.timeoutMs * (stage.retries + 1)` across all 10 stages) is what the backstop arms against, plus the 300,000ms margin — currently ≈3h37m and ≈3h42m respectively, against the stage timeouts wired today. Rule (A12): this figure is ALWAYS re-derived from the stage table at runtime by calling `computeRunCapMs()`, never hardcoded in code or docs — stage timeouts change, and a hardcoded figure silently goes stale the moment they do without anything failing loudly to say so. (The previous text here hardcoded two figures, both arithmetic errors. The figure is deliberately not restated here, per this same rule: after D14 deletes the launchd adapter, the only surviving source of truth is `computeRunCapMs()` itself, evaluated against the stage table wired at the time.)

It exists because a wedged child cannot enforce its own cap — an internal guard cannot kill a wedged process, which is the entire point of an external backstop. This implements the external-backstop half of D3's "makes the run-cap watchdog a simple child kill".
...
The tick loop uses `setInterval(tick, 30_000)` where `tick` is `async`. Node's timer does not skip a scheduled firing merely because the previous invocation's returned promise is still pending, so a still-running batch of sequential child spawns (§6.4) must never be entered twice concurrently by the daemon's own timer — that is what the reentrancy guard below is for.

**Heartbeat precedes the guard (A15.1)**: the pidfile's `lastTickAt` write (D22, §6.2) is the FIRST statement of the `tick` callback, BEFORE the reentrancy check. It therefore executes on every timer firing, including firings the guard immediately short-circuits because a child run is still in flight. Only after the heartbeat write does `tick` run the in-memory boolean guard: `if (ticking) return; ticking = true; try { ... } finally { ticking = false; }`.

Consequence, stated plainly: a busy daemon's heartbeat stays fresh for the entire duration of a multi-hour child run (§6.5's backstop tolerates up to ~3h37m under current defaults), and that is precisely what makes the 5-minute staleness threshold (D22) safe. Without this ordering, every tick fired during a run would short-circuit on `ticking` before ever reaching the write, `lastTickAt` would freeze for the run's entire duration, and a concurrent `serve start` would judge a live, healthy, busy daemon stale and start a second one — the exact failure D22 exists to prevent.

**Heartbeat write failure is non-fatal (A15.3)**: because the write now runs unconditionally on every tick, a throw from it (e.g. `ENOSPC`, `EACCES`) would otherwise escape the timer callback as an uncaught exception and kill the daemon — a domain-1 blast radius (§9.2) for what is really a domain-2-shaped problem (one degraded operation), the same class A7 already closed for spawn `error` events. The heartbeat write is therefore wrapped in a `try`/`catch`: on failure, the error is logged to `daemon.log` and swallowed, and the tick proceeds to the reentrancy check as normal. Accepted consequence: repeated heartbeat write failures leave `lastTickAt` frozen, so the daemon will eventually be judged stale (D22) and stolen from by a later `serve start` — which is the correct outcome for a daemon that cannot persist its own state.

### 6.7 Autostart (darwin only, D20)

`jobbunny autostart enable` FIRST performs the same D15 migration scan §8 describes for `serve start` (A5, D15, D20): lists `~/Library/LaunchAgents/`, matches against `^com\.jobbunny\.\d{4}\.plist$`, and, on any match, refuses interactively with the identical ready-to-paste cleanup block §8 shows — before writing anything. Without this, a user who runs `autostart enable` before ever running `serve start` would only discover the conflict at the NEXT login, when the LaunchAgent's own `serve start` invocation refuses and the refusal lands in `daemon.log`, invisible until the user goes looking; performing the same check at `enable` time surfaces it interactively, when the user can act on it immediately.

Only once that scan passes does `autostart enable` write ONE LaunchAgent, `~/Library/LaunchAgents/com.jobbunny.autostart.plist`, with `RunAtLoad: true`, no `StartCalendarInterval`, and a program array invoking `jobbunny serve start` (the foreground/parent form — it detaches itself per §6.1/§6.3). It carries zero schedule knowledge — the daemon's tick loop remains the only interpreter of `times`/`weekdays`/`graceMinutes` (§2). `jobbunny autostart disable` unloads it (`launchctl bootout gui/<uid>/com.jobbunny.autostart`) and removes the plist. Both subcommands are darwin-only; on win32/linux they print "not supported on this platform" and exit nonzero.
...
The migration check (D15, §8) is narrowed to `^com\.jobbunny\.\d{4}\.plist$` specifically so it never matches `com.jobbunny.autostart.plist` — see §8 for the full reasoning. `autostart enable`'s own scan (above) uses this identical narrowed pattern.
...
**Second case (A1, D22)**: if the recorded pid IS alive but `lastTickAt` is older than 5 minutes, the command instead prints `warning: jobbunny daemon appears wedged (no tick in over 5 minutes) — scheduled runs may not fire`, distinct wording from the not-running case, since the process is alive but its tick loop has stalled — this is what makes §11's rare reboot/pid-recycling window loud rather than silent. A pidfile that fails to parse is neither case: per §6.2's reader rule, the command retries the read once and, if it still fails to parse, suppresses this warning entirely rather than misreporting either state.

A machine that has just woken from sleep can transiently trigger this warning for up to one tick interval (A15.5) — `setInterval` does not fire during suspend (D4), so a freshly-woken, healthy daemon can legitimately show a stale heartbeat for that long. This warning is advisory only and deliberately does NOT re-check before printing, unlike the steal path in §6.2 (A15.4): the steal path mutates state — it decides whether to take over a pidfile — and therefore must re-check before acting, whereas a stderr warning that turns out to be a false positive costs nothing, so delaying it to avoid one is not worth the code.
...
- **`runs.log`** — every spawned run child's stdout/stderr, same append-fd mechanism, passed as that child's `stdio` in §4.3 step 5c. Because D6 mandates sequential execution, only one child writes to `runs.log` at a time, so a single shared file cannot interleave — a per-run log file is unnecessary.
...
**Rotation (A6)**: the two log files rotate on different triggers, deliberately asymmetric — not an oversight.

- **`runs.log`** rotates BEFORE EACH child spawn (§4.3 step 5c), not only at daemon start. With D20 shipping darwin autostart, the success case is a daemon running for months, so start-only rotation would let `runs.log` grow unboundedly. Immediately before each spawn, the daemon checks `runs.log`'s current size — the safe quiet point, since D6's sequential-execution guarantee means no child is writing to it at that instant. If it is over 10 MB: close the fd, rename it to `runs.log.1` (replacing any existing `.1`), reopen a fresh `runs.log`, then spawn using the new fd.
- **`daemon.log`** keeps start-only rotation — checked and rotated once, at daemon start, exactly as originally specified; it is never rotated mid-lifetime. Two reasons this asymmetry is correct: (1) `daemon.log`'s fd is fixed at the detached child's `spawn()` call (§6.1, §6.3) and cannot be re-handed to a running process the way `runs.log`'s fd is re-opened between sequential child spawns; (2) renaming a file with an open handle fails on Windows, so rotating a file the daemon is actively writing to is unsafe cross-platform — `runs.log`'s per-spawn quiet point never has an open handle at rotation time, `daemon.log` always does.

**`daemon.log` write discipline (A6, additional rule not in the original review)**: the daemon writes to `daemon.log` on EVENTS only — spawn, child exit, a skipped expired slot (A3), a spawn `error` (A7), daemon start, and `serve stop`'s shutdown — never once per 30s tick. This is what keeps start-only rotation acceptable for `daemon.log` specifically: a near-silent daemon's log grows slowly even over months of ticking, because ticks that find nothing owed write nothing at all. This asymmetry would not hold for `runs.log`, which accumulates a full run's stdout/stderr on every served slot regardless of how quiet the daemon itself is.
...
The mirror-image risk is **pid reuse on the Chrome pid file**. If Chrome dies out-of-band without the pid file being cleared (so the recorded pid is stale), *and* the OS reassigns that exact pid to an unrelated live process, *and* something is still listening on the CDP port at decision time, then the `>24h` recycle branch (§7.4) sends `SIGTERM`/`SIGKILL` to that unrelated process — `pidIsAlive` says "alive", the recycle path trusts it, and the kill lands on a stranger. A port-match guard cannot help: the pid file's recorded `port` matches the port we just probed by construction, so comparing them proves nothing. Accepted because the conjunction is narrow (all three conditions must hold simultaneously, and a stale-pid file is itself already the uncommon case), and the only robust corroboration — asking the OS which process actually holds the CDP socket, or what the candidate pid's command line is — would reintroduce exactly the `lsof`/`ps` shell-outs D12 removes, trading a narrow theoretical mis-kill for the cross-platform brittleness the whole port exists to eliminate.
...
| A spawned child process (run-child, §4.3 step 5c, or daemon-child, §6.1) emits an `error` event instead of exiting (e.g. `ENOENT`, `EMFILE`) | Fail-soft (A7) | Run-child spawn error is treated exactly as a nonzero child exit (previous row) — the ledger append from §4.3 step 5b (D19) already prevents a retry storm. Daemon-child spawn error in `serve start`'s parent follows the same path as the existing already-dead-at-2s check (§6.1): the parent removes the pidfile, prints the tail of `daemon.log`, and exits 1. Without a handler, an `error` event on a spawned child is an uncaught exception that kills the daemon — a domain-1 blast radius ("all future runs", §9.2) for what should be a domain-2, or parent-exit-1, problem. |
| The heartbeat write (§6.2, D22) — the first statement of the tick callback, before the reentrancy guard (§6.6, A15.1) — throws (e.g. `ENOSPC`, `EACCES`) | Fail-soft (A15.3) | The write is wrapped in `try`/`catch`: the error is logged to `daemon.log` and swallowed, and the tick proceeds to the reentrancy check normally. Without this, the throw would escape the timer callback as an uncaught exception and kill the daemon — a domain-1 blast radius for a domain-2-shaped problem, the same class A7 already closed for spawn errors. Accepted consequence: repeated failures leave `lastTickAt` frozen, so the daemon is eventually judged stale (D22) and stolen from by a later `serve start` — the correct outcome for a daemon that cannot persist its own state. |
| The daemon pidfile is stale (dead pid, or `lastTickAt` older than 5 minutes — D22) at `serve start` | Fail-soft (recoverable) | The heartbeat-based staleness rule (§6.2, D22) — not `run_lock.ts`'s 4-hour `DEFAULT_MAX_AGE_MS` age rule, which would wrongly judge every daemon older than four hours stale (A1) — judges the pidfile stale; if the recorded pid is alive (not dead), `serve start` re-checks once after 35 seconds before acting (A15.4, §6.2) rather than stealing on first observation. Once confirmed stale, the pidfile is deleted and a fresh one is created, and `serve start` proceeds normally. |
...
| Pid reuse on the Chrome pid file (§7.6): if Chrome dies out-of-band leaving the pid file uncleared, AND the OS reassigns that pid to an unrelated live process, AND something is still listening on the CDP port, the `>24h` recycle branch will `SIGTERM`/`SIGKILL` that unrelated process. | Accepted (D12) — a port-match guard cannot help (the recorded port matches the probed port by construction), and the only robust corroboration (asking the OS who owns the socket / what the pid actually is) would reintroduce the `lsof`/`ps` shell-outs D12 exists to remove. The conjunction is narrow enough that paying that price is the worse trade. |
...
| A reboot that completes within 5 minutes of the daemon's last recorded tick (D22), where the OS also recycles the daemon's old pid onto an unrelated process before the daemon restarts, makes the pidfile look live (recycled pid alive, `lastTickAt` still inside the 5-minute window) — autostart's `serve start` (§6.7) refuses once at login, believing a daemon is already running. | Accepted (A1) — self-heals automatically once the heartbeat threshold passes (at most 5 more minutes past the last real tick), and §6.8's wedged-daemon warning makes the interim loud rather than silent. Vanishingly rare: requires both a sub-5-minute reboot AND pid recycling landing on an unrelated process in that exact window. |
| A long-running predecessor child in the same sequential batch (§6.4) can consume a later slot's entire grace window before its turn arrives, so the per-entry revalidation (A3, §4.3 step 5a) skips it — a second, distinct cause of silent grace-window expiry alongside "daemon was down" (§9.1). | Accepted — forced by D6's one-Chrome sequential-execution constraint: two children cannot run concurrently, so a slow predecessor is not itself a bug, and skipping (rather than force-spawning) an expired slot is exactly what D8's "a 09:00 slot can never fire in the evening" requires. |

## 12. Same-change documentation updates

Per D23 and CLAUDE.md's own hard rule — architecture docs are code, updated in the same change that alters behavior, not as a follow-up — this port updates the following in the same change that ships it:

- **`CLAUDE.md`**: the "What this is" line ("macOS only (launchd scheduling, hardcoded Chrome path)" — both clauses become false); the Commands section (`schedule install`/`schedule remove` replaced by `serve start|stop|status` and `autostart enable|disable`, per D14/§4.1); the Profiles section's `schedule install` cross-profile exception (D6/§6.1's `serve` inherits the same "no `--profile`" posture and must be named there instead); the Pipeline architecture section wherever it names the scheduler adapter (D14 deletes `src/adapters/scheduler/launchd/` and `src/ports/scheduler.ts` entirely); the Known limitations section, if any listed limitation there is affected by this port (checked, not assumed).
- **`.claude/agents/explainer.md`**: the scheduling portion of the baked-in KB (the `launchd`/plist architecture this spec replaces), the CLI-surface portion (`schedule install|remove` → `serve start|stop|status` + `autostart enable|disable`), and the adapter-inventory portion (the `scheduler/launchd` family removed, no new adapter family added since the daemon lives in `src/ops/daemon/` — an orchestration layer, not an adapter, per D2/D14's placement).
- **`.claude/agents/executor.md`**: checked for any reference to the scheduler adapter family or the `schedule` subcommands (its placement decision tree currently lists `scheduler` among the port examples and adapter families); updated if found, per the same same-change rule this section itself is closing.
- **`README.md`**: the Windows/Linux manual autostart alternative D20 promises (run `jobbunny serve start` once after login/boot, or register the OS-native "run at login" mechanism by hand — §6.7's exact wording); and the `claude` CLI prerequisite from D13 (documented as a cross-platform prerequisite, not an OS blocker).

This list is the full scope of D23's obligation — no file on it may be left for a follow-up change.
