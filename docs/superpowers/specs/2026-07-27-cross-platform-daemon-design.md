...
| D22 | Daemon pidfile staleness (heartbeat, not age) | Daemon pidfile gains `lastTickAt: string` (ISO 8601), rewritten by the daemon on EVERY 30s tick through the atomic update path (§6.2). Staleness (safe to steal at `serve start`) is: recorded `pid` not alive, OR `lastTickAt` older than 5 minutes (10 missed ticks) — this REPLACES reuse of `run_lock.ts`'s 4-hour `DEFAULT_MAX_AGE_MS` for the daemon pidfile specifically. `run_lock.ts`'s own 4-hour rule remains correct for `run_lock` (a single bounded run) and is simply the wrong rule for a long-lived daemon. | §6.2 previously reused `run_lock.ts`'s `isStale` rule verbatim (pid dead OR `startedAt` older than `DEFAULT_MAX_AGE_MS` = 4h). The daemon writes `startedAt` once and runs for days, so from hour 4 onward every `serve start` would judge a healthy daemon stale and start a second one — which §9.1's "no second daemon is ever allowed" row forbids. A heartbeat ties staleness to liveness (is it still ticking?) rather than to uptime (how old is it?), which is the property that actually matters for a long-lived process. |
...
interface DaemonPidfile {
  ...
  lastTickAt: string;    // ISO 8601 — rewritten by the daemon on EVERY 30s tick (D22),
                          // through the atomic in-place update path below
  inFlight?: {
...
**Atomic in-place updates (A4)**: every one of those in-place updates — `inFlight`, `attempts`, `lastTickAt` — goes through `updateFile`'s temp-file-plus-rename implementation, the exact pattern at `ops/observability/run_folder.ts:155-160`: write `<path>.tmp`, then `rename` over the target. This REPLACES the plain `writeFileSync` (no `wx`) the pidfile's in-place updates originally used: `writeFileSync` truncates the file before writing, and §6.8 makes EVERY `jobbunny` command — including the daemon's own run children — a concurrent reader of this file, so a truncate-then-write left a window where a reader could observe empty or partial JSON. `run_lock.ts` never had this problem because its file is create-once and never updated in place, so "same mechanism as `run_lock.ts`" never covered updates — only the initial `wx` create. This is mandatory rather than nice-to-have specifically because D22's heartbeat writes at least once every 30 seconds for the daemon's entire lifetime, so an in-place update is now a near-constant occurrence, not an occasional one. `attempts` starts as `[]` when the pidfile is first created and is reset to `[]` in place whenever the daemon detects local-date rollover, using the same atomic update path — see §5's "Attempts ledger" note (A9) for why this needs no new field.
...
| The daemon pidfile is stale (dead pid, or `lastTickAt` older than 5 minutes — D22) at `serve start` | Fail-soft (recoverable) | The heartbeat-based staleness rule (§6.2, D22) — not `run_lock.ts`'s 4-hour `DEFAULT_MAX_AGE_MS` age rule, which would wrongly judge every daemon older than four hours stale (A1) — judges the pidfile stale; it is deleted and a fresh one is created, and `serve start` proceeds normally. |
