// scripts/setup/notify_setup.js — guided Telegram setup for one profile.
// Usage: node scripts/setup/notify_setup.js <profile>   (or JOBBUNNY_PROFILE / config.json default)
//
// Mirrors scripts/setup/init.js's idioms (promptMasked / writeEnvKey / ensureToken for the
// shared secret) and scripts/ops/doctor.js's checkCDP() bounded-poll pattern (for chat_id
...
async function resolveProfileArg() {
  ...
  if (!name) {
    throw new Error("Usage: node scripts/setup/notify_setup.js <profile>   (e.g. node scripts/setup/notify_setup.js harish)");
  }
