...
/** In-memory fake mirroring the real SqliteStateStore contract: undefined
 * for a missing key, schema-validated for a present one. */
...
test('load with a stale (different) persisted date returns a fresh empty state', async () => {
  const stateStore = new FakeStateStore();
  stateStore.set(RESUME_STATE_PATH, {
    date: '2026-07-20',
...
test('persist writes the current date + done-map to lanes/linkedin/extract_resume.json', async () => {
  ...
  await state.persist(stateStore);

  const reloaded = await ResumeState.load(stateStore, '2026-07-21');
