...
import { PROPERTIES, STATUS_OPTIONS } from './schema.ts';
...
const PASSED_STATUS = STATUS_OPTIONS[STATUS_OPTIONS.indexOf('Passed')];
...
/** v0 cleanup.js's `cutoffISO` — today minus `daysOld`, as a `YYYY-MM-DD`
 * string (ISO date strings compare lexicographically, so a plain `<`
 * against Date Found's own `date.start` is exact). */
function cutoffISO(daysOld: number): string {
...
export async function archiveStale(
  ...
  const passedCutoff = cutoffISO(policy.passedOlderThanDays);
  const untouchedCutoff = cutoffISO(policy.untouchedOlderThanDays);
