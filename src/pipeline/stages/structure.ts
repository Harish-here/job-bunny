...
function buildPrompt(inputHeaderLine: string, batch: TableRow[]): string {
  return [
    ...
    '- city / country: the input `location` column is the as-posted job location — PREFER it when filling these; if it resolves only a city, INFER the country from that city; only when the `location` column is empty, derive city/country from rawText instead; leave city and country empty only when neither location nor rawText resolves them.',
