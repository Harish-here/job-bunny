...
- ⚠️ P9 in progress (not complete) — fetch-time gate parity CLOSED 2026-07-25, other register items still open. Full detail: `docs/superpowers/plans/2026-07-21-v2-p9-v0-retirement.md`.
  ...
  **Open risk:** LinkedIn's cap is per-URL, so ~21 search URLs still permit ~840 cards; the title gate should cut this hard but the real number is unmeasured until a live run.
  **Still open in P9 (unchanged by this session):** live end-to-end verification (§6, blocked on a scratch Notion DB id — no v2 run has completed end to end, Notion WRITE path unverified against the real API); the undiagnosed `farm` harvest of 0 jobs in 227s on 2026-07-25; cutover (§7, not started — v0's launchd jobs still live); v0 deletion (§8, gated on the ≥7-day green-run soak); CLAUDE.md/README.md rewrites (deliberately deferred until after cutover).
